...
router.post("/show-accepted", auth, async (req, res) => {
	const { _id } = req.user;
	try {
		console.log("token");
		console.log(_id);
		...
		let project = await Project.findOne({$and: [
				{ $or: [{acceptedUser: _id}, {mentor: _id}] },
				{ isAccepted: true }
			]})
			.populate({ path: "mentor", model: "User" })
			.populate({ path: "acceptedUser", model: "User" })
		...
		// let project = await Project.findOne({ acceptedUser: _id, isAccepted: true })
			// .populate({ path: "mentor", model: "User" })
			// .populate({ path: "acceptedUser", model: "User" })
			// .populate({ path: "apply.mentee", model: "User" });
